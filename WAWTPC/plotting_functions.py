...
def plotEvent(data, model):
    ...
    for iProj in range(0,3):
        ...
        rois = find_ROIs(data, thr=10, size_thr=10)
        #plot_ROIs(rois, axis)
        sy, sx = rois[0]['slice']
...
def find_ROIs(img, thr=10, size_thr=0):
    ...
    x = ndimage.binary_dilation(x, iterations=3)# Why dilatation used?
...
def plotLengthPull(df, partIdx):
    ...
    axes[0].hist(pull, bins=40, label=label);
    axes[0].set_xlabel("RECO-GEN [mm]")

    axes[1].hist(pull, bins=40, label=label);
    axes[1].set_xlabel("RECO-GEN [mm]")
    axes[1].set_yscale('log')
...
def plotOpeningAngleCos(df):
    ...
    axes[0].hist(RECO_cosAlpha, bins=np.linspace(-1, -0.995, 40), alpha=0.5, label="NN");
    axes[0].hist(GEN_cosAlpha, bins=np.linspace(-1, -0.995, 40), alpha=0.8, label="true");
